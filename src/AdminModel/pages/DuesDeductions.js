...
function DuesDeductions() {
  const dispatch = useDispatch();
  const { GetAllDues } = bindActionCreators(dataActionCreators, dispatch);
  ...
  const { dues, paginate, isLoading } = data;
  ...
  useEffect(() => {
    ...
    setCurrentItems(dues?.slice(itemOffset, endOffset));
    setPageCount(Number(paginate?.total));
  }, [itemOffset, itemsPerPage, dues]);

  // Invoke when user click to request another page.
  const handlePageClick = (event) => {
    console.log("event", event);

    GetAllDues(true, event.selected + 1, limit);
  ...
  //     if(search){
  //       console.log('search', search)
  //        GetAllDues(true, 1, 15, `&search=${search}`)
  //     }else{
  //       GetAllDues(true, 1, 15, null)
  //     }
  // }, 800)
                              ...
                              <td className="px-1 py-3 border-b border-gray-200 bg-white text-sm">
                                <div className="flex">
                                  <div className="flex-shrink-0 w-10 h-10">
                                    {due?.user?.profile_image ? (
                                      <img
                                        className="w-full h-full rounded-full"
                                        src={due?.user?.profile_image}
                                        alt=""
                                      />
                                    ) : (
                                      <>
                                      {due?.user?.name?.split(' ')[0] === "Mr." ? (
                                        <img
                                        className="w-full h-full rounded-full"
